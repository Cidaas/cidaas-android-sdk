...
import de.cidaas.sdk.android.service.entity.notificationentity.getpendingnotification.NFCSignObject;
import timber.log.Timber;

import static android.util.Base64.URL_SAFE;

public class U2F_V2 implements Serializable {
    ...
    public String enroll(String jsonRequest, String origin) throws JSONException, IOException, APDUError {
        ...
        if (!request.getString("version").equals(Cidaas.FIDO_VERSION)) {
        ...
        response.put("registrationData", Base64.encodeToString(resp, 0, resp.length, URL_SAFE));
        response.put("clientData", Base64.encodeToString(clientDataString.getBytes(Charset.forName("ASCII")), URL_SAFE));

        return response.toString();
    }

   /* public String sign(NFCSignObject nfcSignObject, String origin) throws JSONException, IOException, APDUError {
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("keyHandle", nfcSignObject.getRegisteredKeys()[0].getKeyHandle());
        jsonObject.put("appId", origin);
        jsonObject.put("challenge", nfcSignObject.getChallenge());
        jsonObject.put("version", Constants.FIDO_VERSION);
        ObjectMapper objectMapper=new ObjectMapper();
        String jsonRequest= objectMapper.writeValueAsString(jsonObject);
       // String jsonRequest = jsonObject.toString();
        Timber.d("siging data" + jsonRequest);
        JSONObject request = (JSONObject) new JSONTokener(jsonRequest).nextValue();
    ...
    public String sign(NFCSignObject nfcSignObject, String origin) throws JSONException, IOException, APDUError {
        if (nfcSignObject != null && nfcSignObject.getRegisteredKeys() != null && nfcSignObject.getRegisteredKeys().length > 0 && nfcSignObject.getRegisteredKeys()[0].getVersion().equals(Cidaas.FIDO_VERSION)) {
            byte[] appParam = DigestUtils.sha256(nfcSignObject.getAppId());
            byte[] keyHandle = Base64.decode(nfcSignObject.getRegisteredKeys()[0].getKeyHandle(), Base64.URL_SAFE);

            JSONObject clientData = new JSONObject();
            clientData.put("typ", "navigator.id.getAssertion");
            clientData.put("challenge", nfcSignObject.getChallenge());
            clientData.put("origin", origin);
            String clientDataString = clientData.toString();
            byte[] clientParam = DigestUtils.sha256(clientDataString);
            ...
            response.put("signatureData", Base64.encodeToString(resp, 0, resp.length, Base64.URL_SAFE));
            response.put("clientData", Base64.encodeToString(clientDataString.getBytes(Charset.forName("ASCII")), Base64.URL_SAFE));
            response.put("challenge", nfcSignObject.getChallenge());
            response.put("appId", nfcSignObject.getAppId());
    ...
    private byte[] send(byte[] apdu) throws IOException, APDUError {
        ...
        while ((status & 0xff00) == 0x6100) {
            ...
            Timber.d("REQ ", "" + new String(Hex.encodeHex(cmd)));
            Timber.d("RESP", "" + new String(Hex.encodeHex(resp)));
